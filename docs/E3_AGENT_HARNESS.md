...
6. Emit a privacy-safe Langfuse trace with bounded summaries, route/tool names, status and duration. Raw ERP content and image/base64 payloads remain excluded by default.
...
Decision: keep the small application-owned Chat Completions adapter and use only `kimi-k2.6` at `https://api.moonshot.ai/v1`. Kimi thinking is disabled for this rollout, tool schemas follow Moonshot's strict MFJS subset, and current-turn JPEG/PNG/WebP images are sent as base64 `image_url` content parts.
