...
export function saveEntry(input: Record<string, unknown>, user: ErpUser, members: string[], attachment?: { name: string; bytes: Uint8Array }) {
  ...
  return mutate(async bindings => {
    ...
    if (entry.kind === "task") {
      const recipients = taskNotificationRecipients(entry);
