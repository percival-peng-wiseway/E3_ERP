...
const DOMAIN_PATTERNS: Array<[AgentDomain, RegExp]> = [
  ...
  ["knowledge", /\b(?:policy|procedure|knowledge|faq|document|eligib)\w*\b|政策|流程|知识库|资格/iu],
...
export function requiredIdentifierClarification(message: string, domains: readonly AgentDomain[]): string | null {
  if (domains.includes("inventory") && !/\b[A-Z0-9][A-Z0-9._-]{2,}\b/u.test(message)) {
    return "请提供要查询的 SKU（如有需要也请提供仓库）。";
  }
  if (domains.includes("project") && !/(?:\b(?:PRJ|PROJ|PROJECT)[_-][A-Z0-9-]{2,}\b|\b(?:PRJ|PROJ)\d{2,}\b)/iu.test(message)) {
    return "请提供项目编号。";
  }
