...
The knowledge base requires the private `e3-knowledge` Cloudflare Vectorize index and Workers AI. Their bindings are `KNOWLEDGE_VECTORS` and `AI`; neither is exposed to the browser. Long-running ingestion uses the `e3-knowledge-indexer` Cloudflare Workflow exported by `worker.ts` and bound as `KNOWLEDGE_INDEX_WORKFLOW`. Create the 1,024-dimension cosine index and its `access_scope` metadata index before deployment. Exact commands and the migration/reindex procedure are in [docs/KNOWLEDGE_BASE.md](./docs/KNOWLEDGE_BASE.md).
...
| `MOONSHOT_API_KEY` | Kimi K2.6 Agent answers, image understanding and Business Agent | Required for model-backed requests; create in the Moonshot console and store only as a Cloudflare Secret |
...
- `KNOWLEDGE_VECTORS` (Cloudflare Vectorize) stores disposable 1,024-dimension chunk embeddings generated by the `AI` Workers AI binding. Files remains authoritative in `ERP_DB` + `ERP_FILES`; D1 migration `0005_knowledge_base.sql` stores document metadata, authoritative chunk text/locators, active generations and leased jobs. Kimi K2.6 is the only answer generator.
...
Knowledge deployment order is: create `e3-knowledge` and its `access_scope` metadata index, generate/check binding types with `npm run cf-typegen`, apply migration `0005`, build, then deploy. Existing documents created under the former provider must be run through **Vectorize again** once. Rolling the application back does not require deleting Files or vectors; leave the D1 tables intact and follow the knowledge-base runbook.
...
Prefer the `MOONSHOT_API_KEY` Cloudflare Secret for long-lived Agent credentials. The settings UI and save API accept a replacement key plus a trusted `china` or `international` region and store them server-side. The server maps those choices only to the official `https://api.moonshot.cn/v1` and `https://api.moonshot.ai/v1` endpoints, verifies that the selected key can access `kimi-k2.6`, and rejects arbitrary endpoints or model changes. Local Agent settings—including any existing key—are never copied by the migration script.
...
For a migration from the retired provider, validate the Kimi compatibility spike and a staff-only deployment first. Then remove any saved model settings once from **Settings → Agent Settings** (or save the Kimi key to overwrite the old document) and delete the unused secret with `npx wrangler secret delete DEEPSEEK_API_KEY`. Do not delete the old secret before the Kimi deployment has passed its live checks.
