...
Prefer the `MOONSHOT_API_KEY` Cloudflare Secret for long-lived Agent credentials. The settings UI and save API accept only a replacement key and store it server-side; they expose no endpoint or model controls. Local Agent settings—including any existing key—are never copied by the migration script. The runtime pins the endpoint and model to `https://api.moonshot.ai/v1` and `kimi-k2.6`.
